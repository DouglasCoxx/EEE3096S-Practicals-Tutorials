...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  uint8_t currentState = HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0); // read current state

	  if(prevState == GPIO_PIN_RESET && currentState == GPIO_PIN_SET) {
		  ...
		  HAL_Delay(100); // delay to debounce button
	  }

	  prevState = currentState; // Store the current state for comparison in the next loop
